...
float checkGPU1(unsigned char * d_result_pixels, int radius, int k);
float cudaPallel1(unsigned char * d_result_pixels, int radius);
float checkGPU2(unsigned char * d_result_pixels, int radius, int k);
float cudaPallel2(unsigned char * d_result_pixels, int radius);
...
int main(int argc, char ** argv)
{
	...
	float* ans2 = new float[N];

	for (int i = 1; i < radius; i++) {
		rs[i - 1] = i;
		ans1[i - 1] = checkGPU1(d_result_pixels, i, 100);
		ans2[i - 1] = checkGPU2(d_result_pixels, i, 100);
		std::cout << i << std::endl;
	}
...
float checkGPU1(unsigned char * d_result_pixels, int radius, int k) {
	...
	for (int i = 0; i < k; i++) {
		time += cudaPallel1(d_result_pixels, radius);
	}
...
float checkGPU2(unsigned char * d_result_pixels, int radius, int k) {
	...
	for (int i = 0; i < k; i++) {
		time += cudaPallel2(d_result_pixels, radius);
	}
...
float cudaPallel2(unsigned char * d_result_pixels, int radius) {
	...
	cudaEventRecord(start, 0);
	//----
	BoxBlur_kernel << <gridSize, blockSize >> >(d_result_pixels, radius, width, height);
	//negative_kernel << < grid, block >> >(d_result_pixels, width, height);
